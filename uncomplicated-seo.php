...
function uncomplicated_seo_print_header(){
    ...
    if( is_home() || is_front_page() ){
        ...
        // Get the description of the custom metabox
        ...
        if( empty($uc_options['description']) ){
            if(!empty($web_info->post_excerpt)){
                // .. try to get if from the excerpt
                $uc_options['description'] = $web_info->post_excerpt;
            }else{
                //.. if there is also no excerpt,
                // leave it blank
                if(empty($uc_options['description'])){
    ...
    if($uc_options["twittercard"] == '1'){
        uncomplicated_seo_twitter_card( 
            $uc_options['twitter'],
            $uc_options['title'],
            $uc_options['description'],
            $uc_options['url'],
    ...
    if( $uc_options["socialicons"] == '1' && is_single() ){
        ...
        add_filter('the_content', 'uncomplicated_seo_after_content');
    }
...
function uncomplicated_seo_open_graph($title, $description, $type, $image, $url, $post_author, $published_time, $modified_time){

    // Since it can't be left blank, take the blog description
    if( $description == '' ){
...
// Print Social Icons
function uncomplicated_seo_after_content( $content ) {
	global $post;
